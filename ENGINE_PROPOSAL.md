...
**The cause is now established, and it took three wrong answers to get there.**

Measured on the affected machine, from one row of one insert: the server recorded the reading
at `19:52:51.444Z`; `capturedAt`, a `Date` the application wrote, read back correct to 444ms;
`createdAt`, generated by MySQL's `DEFAULT (now())`, read back as `20:52:51.000Z`. The local
wall clock, returned and taken for UTC. **Values the application writes round-trip correctly.
Values the database generates come back an hour out on a machine whose timezone is not UTC.**

Which layer performs that conversion is not established and does not need to be, because the
fix does not depend on it: `saveReadingSession` now takes one clock reading, uses it to judge
the device's clock and writes it as `createdAt`, down the same path `capturedAt` already
proved correct on that very row. It is also the more honest record - the id is minted from
that instant a line earlier, so the row agrees with itself instead of carrying a second clock
nothing else in it has seen.

**Still open, and larger than the fix.** Thirty-two columns across the schema are
`.defaultNow()` or `.onUpdateNow()`, and every one of them is subject to the same hour on the
same machine: `updatedAt` on most tables, `lastSignedIn` on users, and `createdAt` on every
table but the one fixed here. None of them is load-bearing for the demo and none is worth
touching two days out, but any figure derived from them - anything counting "this week", any
audit trail, anything comparing a stored timestamp against the current time - is an hour out
before it starts. The choices after the final are to pin the connection's timezone, which
changes how every existing row reads and so needs its own migration plan, or to keep doing
what `saveReadingSession` now does and let the application supply the instant. The second is
narrower and does not rewrite history.

**The three wrong answers are the lesson, not a footnote.** First it was attributed to
MySQL's session timezone differing from the machine's; measured, that was refuted - both
`SYSTEM`, Europe/London, `NOW()` exactly on the clock. Then the instrument was rebuilt to
measure the column rather than `NOW()`, and it sampled the twenty newest rows and reported
that all of them agreed - but the suite inserts far more than twenty rows after the failing
test, so the row under investigation was outside the window. A sample that cannot contain the
case answers a different question while sounding certain.

What finally settled it was the assertion carrying its own inputs. `expected 3599498 to be
less than 60000` is a difference with nothing attached; the same assertion reporting
`createdAt`, the insert window, `capturedAt` and the id's mint time gave the answer in one
run. **An assertion that discards the values it compared is a diagnostic that cannot be read,
and this one cost three rounds.** The same applies to the diagnostic's own sampling: it now
scans every row, names the ones actually out, and leaves the deliberately backdated seed rows
out of the measurement rather than reporting them as faults.
