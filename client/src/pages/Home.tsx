...
function createGuidedReport(story: Story, transcript: string, durationSeconds: number, mode: AssessmentMode, wordStates: WordState[]): Report {
  ...
  const suggestedPractice = story.id === "kite" ? ["glimmered", "gentle"] : story.id === "seed" ? ["whispered", "sunflower"] : ["afternoon", "appeared"];
  const skippedWords = wordStates.filter(state => state.status === "incorrect" && state.attempts >= 3).map(state => state.text);
  const practiceWords = Array.from(new Set([...skippedWords, ...suggestedPractice])).slice(0, 3);
  ...
  const accuracy = expected.length ? Math.round((correctWords / expected.length) * 100) : 0;
  const safeDuration = Math.max(durationSeconds, 1);
  const pace = Math.round((correctWords / safeDuration) * 60);
  return { transcript, mode, accuracy, firstPassAccuracy: accuracy, pace, correctWords, totalWords: expected.length, durationSeconds: safeDuration, practiceWords: mode === "MONTHLY_ASSESSMENT" ? [] : practiceWords, retrySummary, selfCorrections, modelWords: mode === "GUIDED_PRACTICE" ? wordStates.filter(state => state.status === "incorrect" && state.attempts >= 2).map(state => state.text) : [], childMessage: mode === "MONTHLY_ASSESSMENT" ? "Your monthly reading check was saved. Your teacher will review it with you." : selfCorrections.length ? "You had another go at a tricky word. That is a smart reader move." : "Your reading practice was saved. You can choose one small word activity next.", nextStep: mode === "MONTHLY_ASSESSMENT" ? "No correction prompts were used during this first-pass reading check." : `Try “${practiceWords[0]}” slowly, then read the whole sentence with a smooth voice.`, transcriptionStatus: "guided" };
}
...
export default function Home() {
  ...
  const saveSession = trpc.readerLeader.sessions.save.useMutation({ onSuccess: data => { setSavedSessionId(data.session.id); setHasSavedRecording(Boolean(data.session.audioStorageKey)); void utils.readerLeader.sessions.childProgress.invalidate(); void utils.readerLeader.dashboards.teacher.invalidate(); void utils.readerLeader.dashboards.parent.invalidate(); } });
  ...
  useEffect(() => {
    if (readingState !== "listening" || recognitionStatus !== "listening") {
      setHesitationHint(false);
      return;
    }
    const timer = window.setInterval(() => {
      if (lastRecognitionAtRef.current && Date.now() - lastRecognitionAtRef.current > 5_500) setHesitationHint(true);
    }, 700);
  ...
  const processAndSave = trpc.readerLeader.sessions.processAndSave.useMutation({ onSuccess: data => { const recordingSaved = Boolean(data.session.audioStorageKey); setSavedSessionId(data.session.id); setHasSavedRecording(recordingSaved); finishWithReport({ ...data.analysis, transcriptionStatus: data.transcriptionStatus, hasRecording: recordingSaved }, false); void utils.readerLeader.sessions.childProgress.invalidate(); void utils.readerLeader.dashboards.teacher.invalidate(); void utils.readerLeader.dashboards.parent.invalidate(); }, onError: () => { finishWithGuidedTranscript(); toast("Your recording could not be saved. Your practice report is still available."); } });
  ...
  function beginRecognition() {
    ...
    recognition.onerror = (event: any) => { if (event.error === "not-allowed" || event.error === "service-not-allowed") { recognitionDesiredRef.current = false; setRecognitionStatus("unavailable"); return; } if (event.error !== "aborted" && event.error !== "no-speech") toast("Live words paused briefly. Keep reading or use the guided practice option."); };
    recognition.onend = () => { if (recognitionDesiredRef.current) { setRecognitionStatus("processing"); window.setTimeout(() => { if (recognitionDesiredRef.current) beginRecognition(); }, 80); } };
    try { recognition.start(); recognitionRef.current = recognition; setRecognitionStatus("listening"); } catch { setRecognitionStatus("processing"); /* A current browser recognition instance is already ending; onend performs the safe restart. */ }
  }
...
function ReadingView({ story, storyWords, processedWords, state, recognitionStatus, transcript, isProcessing, assessmentMode, wordStates, hesitationHint, onMoveOn, onBack, onStart, onPauseResume, onComplete }: { story: Story; storyWords: string[]; processedWords: number; state: ReadingState; recognitionStatus: RecognitionStatus; connectionStatus: ConnectionStatus; transcript: string; isProcessing: boolean; coachMoment: CoachMoment; assessmentMode: AssessmentMode; wordStates: WordState[]; hesitationHint: boolean; onModeChange: (mode: AssessmentMode) => void; onWordAttempt: (correct: boolean) => void; onMoveOn: () => void; onBack: () => void; onStart: () => void; onPauseResume: () => void; onRestart: () => void; onComplete: () => void; onGuidedComplete: () => void; onCoach: (moment: CoachMoment) => void }) {
