...
describe("Irish English variation support", () => {
  it("keeps the reviewed variation opt-in and bounded", () => {
    ...
    expect(matchExpectedReadingWord("caught", "cot", IE)).toEqual({ matches: true, provisionalIrishEnglish: true, feature: "cot-caught-merger", source: "built_in" });
  ...
  it("accepts an educator-approved variant for the class and marks its source", () => {
    const approved = [{ expectedWord: "yellow", recognisedVariant: "yella" }];
    expect(matchExpectedReadingWord("yellow", "yella", IE, approved)).toEqual({ matches: true, provisionalIrishEnglish: true, source: "educator_approved" });
    expect(matchExpectedReadingWord("yellow", "yella", IE)).toEqual({ matches: false, provisionalIrishEnglish: false });
    expect(matchExpectedReadingWord("yellow", "yella", STD, approved)).toEqual({ matches: false, provisionalIrishEnglish: false });
  });
});
