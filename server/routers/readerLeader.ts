...
export const readerLeaderRouter = router({
  ...
  sessions: router({
    processAndSave: protectedProcedure.input(z.object({
      ...
      const session = await saveReadingSession(tenantScope(ctx), { childProfileId: input.childProfileId, materialId: input.materialId, storyTitle: input.storyTitle, transcript: analysis.transcript, accuracy: analysis.accuracy, wordsCorrectPerMinute: analysis.pace, durationSeconds: analysis.durationSeconds, audioStorageKey, audioStatus, assessmentMode: input.assessmentMode, languageSupport: learnerSettings.languageSupport, interventions, wordStates: analysis.wordStates, wordTimings });
    ...
    save: protectedProcedure.input(z.object({
      ...
      const session = await saveReadingSession(tenantScope(ctx), { childProfileId: input.childProfileId, materialId: input.materialId, storyTitle: input.storyTitle, transcript: analysis.transcript, accuracy: analysis.accuracy, wordsCorrectPerMinute: analysis.pace, durationSeconds: analysis.durationSeconds, assessmentMode: input.assessmentMode, languageSupport: learnerSettings.languageSupport, interventions: [...input.demoInterventions, ...interventions], wordStates: analysis.wordStates, wordTimings });
      await createProvisionalMatchReviews(tenantScope(ctx), { sessionId: session.id, childProfileId: input.childProfileId, classId: irishVariantContext.classId, matches: analysis.events.filter(event => event.provisionalIrishEnglish && event.recognisedWord).map(event => ({ expectedWord: event.expectedWord, recognisedWord: event.recognisedWord!, source: event.variantSource })) });
