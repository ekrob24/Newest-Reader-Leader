...
/** True only when the absence means something went wrong. "discarded_by_policy" is the
 *  retention commitment working as designed and must never be shown as a fault; "not_captured"
 *  is a plain statement of fact about a reading that sent no audio. */
export function isAudioFailure(status: AudioRetentionStatus) {
  return status === "storage_unavailable" || status === "storage_rejected";
}
...
export function audioAbsenceSummary(status: AudioRetentionStatus) {
  switch (status) {
    ...
    case "discarded_by_policy": return "Recording scored and discarded in the same request, as the retention policy requires";
...
/** The sentence shown where a flagged word's clip would be. The clip makes review better, not
 *  possible: the transcript and the surrounding words still support a decision. */
